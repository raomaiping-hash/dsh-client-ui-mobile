...
- **抽屉式侧栏**——820px 以下侧栏变为左侧抽屉；由官方顶栏右侧「展开侧边栏」按钮打开（跨页面、内嵌不悬浮），遮罩（仅覆盖对话区）与 Esc 关闭，会话切换时自动收起。
...
- 浏览器半部（`client.js`）通过 `package.json` 的 `dsh.client`（`exports["./client"]`）加载。它注入一个 `<style>` 标签，并把一个 `MobileChrome` 组件注册到 `shell.overlay` 插槽（遮罩；侧栏打开沿用官方顶栏右侧「展开侧边栏」按钮），再把一张小卡片注册到 `settings.general.item`（"已安装"提示）。
